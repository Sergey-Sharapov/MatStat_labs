...
N_laplace = 100
lap_scale = 4
laplace = st.laplace(loc=0, scale=lap_scale)
x_laplace = laplace.rvs(N_laplace)
file = open("laplace" + str(lap_scale) + ".txt", 'w')
...
p.append(1 - model.cdf(delta[k - 2]))
p = np.array(p)
print("p.sum = " + str(p.sum()))

hi_square = 0
for i in range(0, k):
    file.write(str(i))
    if i == k - 1:
        file.write('&  ($ ' + str(delta[i-1]) +',\infty$)')
    else:
        if i == 0:
            file.write('&  ($-\infty ,' + str(delta[i]) + '$]')
        else:
            file.write('&  ($'+ str(delta[i-1]) + ',' + str(delta[i]) +'$]')
    file.write('&  ' + str(int(n[i])) + '&  %.4f' % p[i])
    temp_v = ((n[i] - N_laplace * p[i]) ** 2) / (N_laplace * p[i])
    file.write('&  %.4f' % temp_v + '\\\\ \hline\n')
    hi_square += temp_v
...
for i in range(0, k):
    ...
    if i == k - 1:
        ...
        file.write('  delta = ' + str(delta[i]))
    file.write('  n_i = ' + str(n[i]) + '  p_i = ' + str(p[i]))
    temp_v = ((n[i] - N_laplace * p[i]) ** 2) / (N_laplace * p[i])
    file.write('  temp_v = ' + str(temp_v) + '\n')
    hi_square += temp_v
"""